...
resource "lovi_address" "problem-team001-1" {
  subnet_id = lovi_subnet.team001.id
  fixed_ip = "10.160.1.${100 + 1}"

  depends_on = [lovi_subnet.team001]
}

resource "lovi_lease" "problem-team001-1" {
  address_id = lovi_address.problem-team001-1.id

  depends_on = [lovi_address.problem-team001-1]
}

resource "lovi_virtual_machine" "problem-team001-1" {
  name = "team001-${format("%03d", 1)}"
  vcpus = 2
...
resource "lovi_interface_attachment" "problem-team001-1" {
  virtual_machine_id = lovi_virtual_machine.problem-team001-1.id
  bridge_id = lovi_bridge.team001.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t001-${format("%03d", 1)}"
  lease_id = lovi_lease.problem-team001-1.id

  depends_on = [
    lovi_virtual_machine.problem-team001-1,
    lovi_lease.problem-team001-1
  ]
}
resource "lovi_address" "problem-team001-2" {
  subnet_id = lovi_subnet.team001.id
  fixed_ip = "10.160.1.${100 + 2}"

  depends_on = [lovi_subnet.team001]
}

resource "lovi_lease" "problem-team001-2" {
  address_id = lovi_address.problem-team001-2.id

  depends_on = [lovi_address.problem-team001-2]
}

resource "lovi_virtual_machine" "problem-team001-2" {
  name = "team001-${format("%03d", 2)}"
  vcpus = 2
...
resource "lovi_interface_attachment" "problem-team001-2" {
  virtual_machine_id = lovi_virtual_machine.problem-team001-2.id
  bridge_id = lovi_bridge.team001.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t001-${format("%03d", 2)}"
  lease_id = lovi_lease.problem-team001-2.id

  depends_on = [
    lovi_virtual_machine.problem-team001-2,
    lovi_lease.problem-team001-2
  ]
}
resource "lovi_address" "problem-team001-3" {
  subnet_id = lovi_subnet.team001.id
  fixed_ip = "10.160.1.${100 + 3}"

  depends_on = [lovi_subnet.team001]
}

resource "lovi_lease" "problem-team001-3" {
  address_id = lovi_address.problem-team001-3.id

  depends_on = [lovi_address.problem-team001-3]
}

resource "lovi_virtual_machine" "problem-team001-3" {
  name = "team001-${format("%03d", 3)}"
  vcpus = 4
  memory_kib = 1 * 1024 * 1024
  root_volume_gb = 30
  source_image_id = "18295a50-6ec0-4a6f-8dc7-f19d28d2057e"
...
resource "lovi_interface_attachment" "problem-team001-3" {
  virtual_machine_id = lovi_virtual_machine.problem-team001-3.id
  bridge_id = lovi_bridge.team001.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t001-${format("%03d", 3)}"
  lease_id = lovi_lease.problem-team001-3.id

  depends_on = [
    lovi_virtual_machine.problem-team001-3,
    lovi_lease.problem-team001-3
  ]
}
