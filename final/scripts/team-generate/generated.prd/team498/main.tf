...
resource "lovi_address" "problem-team498-1" {
  subnet_id = lovi_subnet.team498.id
  fixed_ip = "10.164.98.${100 + 1}"

  depends_on = [lovi_subnet.team498]
}

resource "lovi_lease" "problem-team498-1" {
  address_id = lovi_address.problem-team498-1.id

  depends_on = [lovi_address.problem-team498-1]
}

resource "lovi_virtual_machine" "problem-team498-1" {
  name = "team498-${format("%03d", 1)}"
  vcpus = 2
...
resource "lovi_interface_attachment" "problem-team498-1" {
  virtual_machine_id = lovi_virtual_machine.problem-team498-1.id
  bridge_id = lovi_bridge.team498.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t498-${format("%03d", 1)}"
  lease_id = lovi_lease.problem-team498-1.id

  depends_on = [
    lovi_virtual_machine.problem-team498-1,
    lovi_lease.problem-team498-1
  ]
}
resource "lovi_address" "problem-team498-2" {
  subnet_id = lovi_subnet.team498.id
  fixed_ip = "10.164.98.${100 + 2}"

  depends_on = [lovi_subnet.team498]
}

resource "lovi_lease" "problem-team498-2" {
  address_id = lovi_address.problem-team498-2.id

  depends_on = [lovi_address.problem-team498-2]
}

resource "lovi_virtual_machine" "problem-team498-2" {
  name = "team498-${format("%03d", 2)}"
  vcpus = 2
...
resource "lovi_interface_attachment" "problem-team498-2" {
  virtual_machine_id = lovi_virtual_machine.problem-team498-2.id
  bridge_id = lovi_bridge.team498.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t498-${format("%03d", 2)}"
  lease_id = lovi_lease.problem-team498-2.id

  depends_on = [
    lovi_virtual_machine.problem-team498-2,
    lovi_lease.problem-team498-2
  ]
}
resource "lovi_address" "problem-team498-3" {
  subnet_id = lovi_subnet.team498.id
  fixed_ip = "10.164.98.${100 + 3}"

  depends_on = [lovi_subnet.team498]
}

resource "lovi_lease" "problem-team498-3" {
  address_id = lovi_address.problem-team498-3.id

  depends_on = [lovi_address.problem-team498-3]
}

resource "lovi_virtual_machine" "problem-team498-3" {
  name = "team498-${format("%03d", 3)}"
  vcpus = 4
  memory_kib = 1 * 1024 * 1024
  root_volume_gb = 30
  source_image_id = "ee12d10d-471e-4939-8650-9302c2ea71c0"
...
resource "lovi_interface_attachment" "problem-team498-3" {
  virtual_machine_id = lovi_virtual_machine.problem-team498-3.id
  bridge_id = lovi_bridge.team498.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t498-${format("%03d", 3)}"
  lease_id = lovi_lease.problem-team498-3.id

  depends_on = [
    lovi_virtual_machine.problem-team498-3,
    lovi_lease.problem-team498-3
  ]
}
