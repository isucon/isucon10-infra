...
resource "lovi_address" "bastion100-team021" {
  subnet_id = lovi_subnet.team021.id
  fixed_ip = "10.160.21.100"
}

resource "lovi_lease" "bastion100-team021" {
  address_id = lovi_address.bastion100-team021.id

  depends_on = [lovi_address.bastion100-team021]
}

resource "lovi_virtual_machine" "bastion100-team021" {
  name = "team021-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team021" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team021.id
  bridge_id = lovi_bridge.team021.id
  average = 125000 // NOTE: 1Gbps
  name = "t021-ba100"
  lease_id = lovi_lease.bastion100-team021.id

  depends_on = [
    lovi_virtual_machine.bastion100-team021,
    lovi_lease.bastion100-team021
  ]
}
resource "lovi_address" "bastion200-team021" {
  subnet_id = lovi_subnet.team021.id
  fixed_ip = "10.160.21.200"
}

resource "lovi_lease" "bastion200-team021" {
  address_id = lovi_address.bastion200-team021.id

  depends_on = [lovi_address.bastion200-team021]
}

resource "lovi_virtual_machine" "bastion200-team021" {
  name = "team021-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team021" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team021.id
  bridge_id = lovi_bridge.team021.id
  average = 125000 // NOTE: 1Gbps
  name = "t021-ba200"
  lease_id = lovi_lease.bastion200-team021.id

  depends_on = [
    lovi_virtual_machine.bastion200-team021,
    lovi_lease.bastion200-team021
  ]
}