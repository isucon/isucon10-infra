...
resource "lovi_cpu_pinning_group" "team028" {
  name = "team028"
  count_of_core = 4
...
resource "lovi_virtual_machine" "problem-team028" {
  ...
  cpu_pinning_group_name = lovi_cpu_pinning_group.team028.name

  depends_on = [
    lovi_cpu_pinning_group.team028
  ]
}
...
resource "lovi_address" "bastion100-team028" {
  subnet_id = lovi_subnet.team028.id
  fixed_ip = "10.160.28.100"
}

resource "lovi_lease" "bastion100-team028" {
  address_id = lovi_address.bastion100-team028.id

  depends_on = [lovi_address.bastion100-team028]
}

resource "lovi_virtual_machine" "bastion100-team028" {
  name = "team028-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team028" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team028.id
  bridge_id = lovi_bridge.team028.id
  average = 125000 // NOTE: 1Gbps
  name = "t028-ba100"
  lease_id = lovi_lease.bastion100-team028.id

  depends_on = [
    lovi_virtual_machine.bastion100-team028,
    lovi_lease.bastion100-team028
  ]
}
resource "lovi_address" "bastion200-team028" {
  subnet_id = lovi_subnet.team028.id
  fixed_ip = "10.160.28.200"
}

resource "lovi_lease" "bastion200-team028" {
  address_id = lovi_address.bastion200-team028.id

  depends_on = [lovi_address.bastion200-team028]
}

resource "lovi_virtual_machine" "bastion200-team028" {
  name = "team028-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team028" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team028.id
  bridge_id = lovi_bridge.team028.id
  average = 125000 // NOTE: 1Gbps
  name = "t028-ba200"
  lease_id = lovi_lease.bastion200-team028.id

  depends_on = [
    lovi_virtual_machine.bastion200-team028,
    lovi_lease.bastion200-team028
  ]
}