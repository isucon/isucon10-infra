...
resource "lovi_cpu_pinning_group" "team033" {
  name = "team033"
  count_of_core = 4
...
resource "lovi_virtual_machine" "problem-team033" {
  ...
  cpu_pinning_group_name = lovi_cpu_pinning_group.team033.name

  depends_on = [
    lovi_cpu_pinning_group.team033
  ]
}
...
resource "lovi_address" "bastion100-team033" {
  subnet_id = lovi_subnet.team033.id
  fixed_ip = "10.160.33.100"
}

resource "lovi_lease" "bastion100-team033" {
  address_id = lovi_address.bastion100-team033.id

  depends_on = [lovi_address.bastion100-team033]
}

resource "lovi_virtual_machine" "bastion100-team033" {
  name = "team033-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team033" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team033.id
  bridge_id = lovi_bridge.team033.id
  average = 125000 // NOTE: 1Gbps
  name = "t033-ba100"
  lease_id = lovi_lease.bastion100-team033.id

  depends_on = [
    lovi_virtual_machine.bastion100-team033,
    lovi_lease.bastion100-team033
  ]
}
resource "lovi_address" "bastion200-team033" {
  subnet_id = lovi_subnet.team033.id
  fixed_ip = "10.160.33.200"
}

resource "lovi_lease" "bastion200-team033" {
  address_id = lovi_address.bastion200-team033.id

  depends_on = [lovi_address.bastion200-team033]
}

resource "lovi_virtual_machine" "bastion200-team033" {
  name = "team033-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team033" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team033.id
  bridge_id = lovi_bridge.team033.id
  average = 125000 // NOTE: 1Gbps
  name = "t033-ba200"
  lease_id = lovi_lease.bastion200-team033.id

  depends_on = [
    lovi_virtual_machine.bastion200-team033,
    lovi_lease.bastion200-team033
  ]
}