...
resource "lovi_cpu_pinning_group" "team042" {
  name = "team042"
  count_of_core = 4
...
resource "lovi_virtual_machine" "problem-team042" {
  ...
  cpu_pinning_group_name = lovi_cpu_pinning_group.team042.name

  depends_on = [
    lovi_cpu_pinning_group.team042
  ]
}
...
resource "lovi_address" "bastion100-team042" {
  subnet_id = lovi_subnet.team042.id
  fixed_ip = "10.160.42.100"
}

resource "lovi_lease" "bastion100-team042" {
  address_id = lovi_address.bastion100-team042.id

  depends_on = [lovi_address.bastion100-team042]
}

resource "lovi_virtual_machine" "bastion100-team042" {
  name = "team042-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team042" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team042.id
  bridge_id = lovi_bridge.team042.id
  average = 125000 // NOTE: 1Gbps
  name = "t042-ba100"
  lease_id = lovi_lease.bastion100-team042.id

  depends_on = [
    lovi_virtual_machine.bastion100-team042,
    lovi_lease.bastion100-team042
  ]
}
resource "lovi_address" "bastion200-team042" {
  subnet_id = lovi_subnet.team042.id
  fixed_ip = "10.160.42.200"
}

resource "lovi_lease" "bastion200-team042" {
  address_id = lovi_address.bastion200-team042.id

  depends_on = [lovi_address.bastion200-team042]
}

resource "lovi_virtual_machine" "bastion200-team042" {
  name = "team042-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team042" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team042.id
  bridge_id = lovi_bridge.team042.id
  average = 125000 // NOTE: 1Gbps
  name = "t042-ba200"
  lease_id = lovi_lease.bastion200-team042.id

  depends_on = [
    lovi_virtual_machine.bastion200-team042,
    lovi_lease.bastion200-team042
  ]
}