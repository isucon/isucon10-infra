...
resource "lovi_address" "bastion100-team048" {
  subnet_id = lovi_subnet.team048.id
  fixed_ip = "10.160.48.100"
}

resource "lovi_lease" "bastion100-team048" {
  address_id = lovi_address.bastion100-team048.id

  depends_on = [lovi_address.bastion100-team048]
}

resource "lovi_virtual_machine" "bastion100-team048" {
  name = "team048-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team048" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team048.id
  bridge_id = lovi_bridge.team048.id
  average = 125000 // NOTE: 1Gbps
  name = "t048-ba100"
  lease_id = lovi_lease.bastion100-team048.id

  depends_on = [
    lovi_virtual_machine.bastion100-team048,
    lovi_lease.bastion100-team048
  ]
}
resource "lovi_address" "bastion200-team048" {
  subnet_id = lovi_subnet.team048.id
  fixed_ip = "10.160.48.200"
}

resource "lovi_lease" "bastion200-team048" {
  address_id = lovi_address.bastion200-team048.id

  depends_on = [lovi_address.bastion200-team048]
}

resource "lovi_virtual_machine" "bastion200-team048" {
  name = "team048-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team048" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team048.id
  bridge_id = lovi_bridge.team048.id
  average = 125000 // NOTE: 1Gbps
  name = "t048-ba200"
  lease_id = lovi_lease.bastion200-team048.id

  depends_on = [
    lovi_virtual_machine.bastion200-team048,
    lovi_lease.bastion200-team048
  ]
}