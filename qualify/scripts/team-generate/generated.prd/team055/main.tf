...
resource "lovi_address" "bastion100-team055" {
  subnet_id = lovi_subnet.team055.id
  fixed_ip = "10.160.55.100"
}

resource "lovi_lease" "bastion100-team055" {
  address_id = lovi_address.bastion100-team055.id

  depends_on = [lovi_address.bastion100-team055]
}

resource "lovi_virtual_machine" "bastion100-team055" {
  name = "team055-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team055" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team055.id
  bridge_id = lovi_bridge.team055.id
  average = 125000 // NOTE: 1Gbps
  name = "t055-ba100"
  lease_id = lovi_lease.bastion100-team055.id

  depends_on = [
    lovi_virtual_machine.bastion100-team055,
    lovi_lease.bastion100-team055
  ]
}
resource "lovi_address" "bastion200-team055" {
  subnet_id = lovi_subnet.team055.id
  fixed_ip = "10.160.55.200"
}

resource "lovi_lease" "bastion200-team055" {
  address_id = lovi_address.bastion200-team055.id

  depends_on = [lovi_address.bastion200-team055]
}

resource "lovi_virtual_machine" "bastion200-team055" {
  name = "team055-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "5138fee8-59a1-407a-bb84-2937d9705143"
...
resource "lovi_interface_attachment" "bastion200-team055" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team055.id
  bridge_id = lovi_bridge.team055.id
  average = 125000 // NOTE: 1Gbps
  name = "t055-ba200"
  lease_id = lovi_lease.bastion200-team055.id

  depends_on = [
    lovi_virtual_machine.bastion200-team055,
    lovi_lease.bastion200-team055
  ]
}