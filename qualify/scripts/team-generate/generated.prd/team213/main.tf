...
resource "lovi_address" "problem-team213" {
  count = var.node_count

  subnet_id = lovi_subnet.team213.id
  fixed_ip = "10.162.13.${101 + count.index}"
}

resource "lovi_lease" "problem-team213" {
  count = var.node_count

  address_id = lovi_address.problem-team213[count.index].id

  depends_on = [lovi_address.problem-team213]
}
...
resource "lovi_virtual_machine" "problem-team213" {
  count = var.node_count

  name = "team213-${format("%03d", count.index + 1)}"
  vcpus = 1
...
resource "lovi_interface_attachment" "problem-team213" {
  count = var.node_count

  virtual_machine_id = lovi_virtual_machine.problem-team213[count.index].id
  bridge_id = lovi_bridge.team213.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t213-${format("%03d", count.index + 1)}"
  lease_id = lovi_lease.problem-team213[count.index].id

  depends_on = [
    lovi_virtual_machine.problem-team213,
    lovi_lease.problem-team213
  ]
}
