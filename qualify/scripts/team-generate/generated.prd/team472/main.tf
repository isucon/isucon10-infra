...
resource "lovi_address" "problem-team472" {
  count = var.node_count

  subnet_id = lovi_subnet.team472.id
  fixed_ip = "10.164.72.${101 + count.index}"
}

resource "lovi_lease" "problem-team472" {
  count = var.node_count

  address_id = lovi_address.problem-team472[count.index].id

  depends_on = [lovi_address.problem-team472]
}
...
resource "lovi_virtual_machine" "problem-team472" {
  count = var.node_count

  name = "team472-${format("%03d", count.index + 1)}"
  vcpus = 1
...
resource "lovi_interface_attachment" "problem-team472" {
  count = var.node_count

  virtual_machine_id = lovi_virtual_machine.problem-team472[count.index].id
  bridge_id = lovi_bridge.team472.id
  //average = 12500 // NOTE: 100Mbps
  //average = 37500 // NOTE: 300Mbps
  average = 125000 // NOTE: 1Gbps
  name = "t472-${format("%03d", count.index + 1)}"
  lease_id = lovi_lease.problem-team472[count.index].id

  depends_on = [
    lovi_virtual_machine.problem-team472,
    lovi_lease.problem-team472
  ]
}
