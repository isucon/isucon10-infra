...
resource "lovi_address" "bastion100-team502" {
  subnet_id = lovi_subnet.team502.id
  fixed_ip = "10.165.2.100"
}

resource "lovi_lease" "bastion100-team502" {
  address_id = lovi_address.bastion100-team502.id

  depends_on = [lovi_address.bastion100-team502]
}

resource "lovi_virtual_machine" "bastion100-team502" {
  name = "team502-bastion100"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion100-team502" {
  virtual_machine_id = lovi_virtual_machine.bastion100-team502.id
  bridge_id = lovi_bridge.team502.id
  average = 125000 // NOTE: 1Gbps
  name = "t502-ba100"
  lease_id = lovi_lease.bastion100-team502.id

  depends_on = [
    lovi_virtual_machine.bastion100-team502,
    lovi_lease.bastion100-team502
  ]
}
resource "lovi_address" "bastion200-team502" {
  subnet_id = lovi_subnet.team502.id
  fixed_ip = "10.165.2.200"
}

resource "lovi_lease" "bastion200-team502" {
  address_id = lovi_address.bastion200-team502.id

  depends_on = [lovi_address.bastion200-team502]
}

resource "lovi_virtual_machine" "bastion200-team502" {
  name = "team502-bastion200"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
  source_image_id = "c453a2ef-9865-4b14-bff2-0a78416ebea5"
...
resource "lovi_interface_attachment" "bastion200-team502" {
  virtual_machine_id = lovi_virtual_machine.bastion200-team502.id
  bridge_id = lovi_bridge.team502.id
  average = 125000 // NOTE: 1Gbps
  name = "t502-ba200"
  lease_id = lovi_lease.bastion200-team502.id

  depends_on = [
    lovi_virtual_machine.bastion200-team502,
    lovi_lease.bastion200-team502
  ]
}