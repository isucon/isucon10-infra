...
resource "lovi_address" "bastion-team001" {
  subnet_id = lovi_subnet.team001.id
  fixed_ip = "10.160.1.100"
}

resource "lovi_lease" "bastion-team001" {
  address_id = lovi_address.bastion-team001.id

  depends_on = [lovi_address.bastion-team001]
}

resource "lovi_virtual_machine" "bastion-team001" {
  name = "team001-bastion"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion-team001" {
  virtual_machine_id = lovi_virtual_machine.bastion-team001.id
  bridge_id = lovi_bridge.team001.id
  average = 125000 // NOTE: 1Gbps
  name = "t001-ba"
  lease_id = lovi_lease.bastion-team001.id

  depends_on = [
    lovi_virtual_machine.bastion-team001,
    lovi_lease.bastion-team001
  ]
}