...
resource "lovi_address" "bastion-team016" {
  subnet_id = lovi_subnet.team016.id
  fixed_ip = "10.160.16.100"
}

resource "lovi_lease" "bastion-team016" {
  address_id = lovi_address.bastion-team016.id

  depends_on = [lovi_address.bastion-team016]
}

resource "lovi_virtual_machine" "bastion-team016" {
  name = "team016-bastion"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion-team016" {
  virtual_machine_id = lovi_virtual_machine.bastion-team016.id
  bridge_id = lovi_bridge.team016.id
  average = 125000 // NOTE: 1Gbps
  name = "t016-ba"
  lease_id = lovi_lease.bastion-team016.id

  depends_on = [
    lovi_virtual_machine.bastion-team016,
    lovi_lease.bastion-team016
  ]
}