...
resource "lovi_address" "bastion-team025" {
  subnet_id = lovi_subnet.team025.id
  fixed_ip = "10.160.25.100"
}

resource "lovi_lease" "bastion-team025" {
  address_id = lovi_address.bastion-team025.id

  depends_on = [lovi_address.bastion-team025]
}

resource "lovi_virtual_machine" "bastion-team025" {
  name = "team025-bastion"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion-team025" {
  virtual_machine_id = lovi_virtual_machine.bastion-team025.id
  bridge_id = lovi_bridge.team025.id
  average = 125000 // NOTE: 1Gbps
  name = "t025-ba"
  lease_id = lovi_lease.bastion-team025.id

  depends_on = [
    lovi_virtual_machine.bastion-team025,
    lovi_lease.bastion-team025
  ]
}