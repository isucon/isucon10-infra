...
resource "lovi_address" "bastion-team029" {
  subnet_id = lovi_subnet.team029.id
  fixed_ip = "10.160.29.100"
}

resource "lovi_lease" "bastion-team029" {
  address_id = lovi_address.bastion-team029.id

  depends_on = [lovi_address.bastion-team029]
}

resource "lovi_virtual_machine" "bastion-team029" {
  name = "team029-bastion"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion-team029" {
  virtual_machine_id = lovi_virtual_machine.bastion-team029.id
  bridge_id = lovi_bridge.team029.id
  average = 125000 // NOTE: 1Gbps
  name = "t029-ba"
  lease_id = lovi_lease.bastion-team029.id

  depends_on = [
    lovi_virtual_machine.bastion-team029,
    lovi_lease.bastion-team029
  ]
}