...
resource "lovi_address" "bastion-team034" {
  subnet_id = lovi_subnet.team034.id
  fixed_ip = "10.160.34.100"
}

resource "lovi_lease" "bastion-team034" {
  address_id = lovi_address.bastion-team034.id

  depends_on = [lovi_address.bastion-team034]
}

resource "lovi_virtual_machine" "bastion-team034" {
  name = "team034-bastion"
  vcpus = 1
  memory_kib = 2 * 1024 * 1024
  root_volume_gb = 10
...
resource "lovi_interface_attachment" "bastion-team034" {
  virtual_machine_id = lovi_virtual_machine.bastion-team034.id
  bridge_id = lovi_bridge.team034.id
  average = 125000 // NOTE: 1Gbps
  name = "t034-ba"
  lease_id = lovi_lease.bastion-team034.id

  depends_on = [
    lovi_virtual_machine.bastion-team034,
    lovi_lease.bastion-team034
  ]
}